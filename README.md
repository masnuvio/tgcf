...
## 1-Click Deployment on Ubuntu

You can easily deploy tgcf on any Ubuntu server (VPS) with a single command.

1.  **Get the deployment script:**
    Save the `deploy.sh` file to your server.
...
2.  **Run the script:**
    ...
    - Prompt you to set a password for the Web UI.
    - Start tgcf in a Docker container.

3.  **Access the Web UI:**
    Open your browser and navigate to `http://YOUR_SERVER_IP:8501`.
